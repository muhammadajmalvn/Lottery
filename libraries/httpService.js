...
import { trackfn } from "./tracker";
let apiRoot = process.env.NEXT_PUBLIC_SECRET_BACKEND_URL;
const _socketProtocol = process.env.NEXT_PUBLIC_SECRET_SOCKET + "://" + apiRoot?.replace("https://", "").replace("http://", "") + "ws";

// let API_ENDPOINT = "http://localhost:4005/";

// if (process?.env?.NODE_ENV && process?.env?.NODE_ENV === "production") {
//   API_ENDPOINT = "https://oyster-app-87fsp.ondigitalocean.app/";
// }
  ...
  async get(path, select, filter, sort, offset, limit) {
    ...
    return new Promise((resolve, reject) => {
      if (path) {
        ...
        // Check if the user is logged in before adding Authorization headers
        if (localStorage.user) {
          ...
          if (refreshToken) {
            headers.headers.RefreshToken = refreshToken;
          }
        }

        axiosInstance
          .get(apiRoot + path, headers)
          .then((result) => {
            resolve(result);
          })
          .catch((error) => {
...
export const socketProtocol = _socketProtocol;
