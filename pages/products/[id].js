...
export default function Home() {
    ...
    const { rows: eventProducts, setFilter } = useBackend("event-products/event-details", { noGet: true });
    ...
    useEffect(() => {
      ...
      let handleSingleProductDetails = (e,eventId,productId) => {
        Router.push(`/productView/${eventId}/${productId}`);
      };
  return (
...
{eventProducts?.map((item)=>{
  return(
  <div  className={styles.product_grid_wrap} key={id}>
<div className={styles.product_grid1}>
  <div className={styles.product_grid_img_wrap}>
<img src={item?.product?.imageUrl} className={styles.product_grid_img} />
<label className={styles.product_grid_price}>RS {item?.product?.sellPrice}</label>
...
<label className={styles.product_label}>Draw By Chance Signature {item?.product?.productName}</label>


<label className={styles.product_label1}>Buy our signature {item?.product?.productName} worth Rs. {item?.product?.sellPrice} to stand a chance to participate in this  raffle draw. One pen is equal to one raffle entry.</label>

</div>
</div>

<button className={styles.view_details_btn}  onClick={(e)=>handleSingleProductDetails(e,item?.event?.id,item?.product?.id)}>View Details</button>
